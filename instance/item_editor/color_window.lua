...
function ColorWindow:init(parent, x, y, w, h, skin, name, title)
	...
	self.bound_to = nil;
	self.color_picker = dnkColorPicker(self, "color_picker", 0, 16);
	self._mod_label = dnkLabel(self, "_mod_label", 0, 276, "Currently modifying:");
	self.mod_label = dnkLabel(self, "mod_label", 0, 292, "Nothing");
	self.ok_button = dnkButton(self, "ok_button", 0, 16, "Ok");
	...
	self.apply_color = dnkFloatingList.from_link_list(self, "apply_color", 200, 16, "Apply color", self.color_list);
	self.modify_color = dnkFloatingList.from_link_list(self, "modify_color", 200, 40, "Modify color", self.color_list)
	self.save_color = dnkFloatingList.from_link_list(self, "save_color", 200, 64, "Save color", self.color_list)
